...
Numar nopti: {json_file.get("nights")}\n\n\n'''
    return [output,json_file.get("city")]
...
with open ('transmit.json','r') as json_file:
    ...
    info = parseJSON(recv_data)[0]
    city = parseJSON(recv_data)[1]


with open ('extracted_data_booking.txt','w') as file:
    ...
    file.write("PRETURI:\n")
    file.write(f"Media preturilor locuintelor din {city} este : {avg_price} lei\n")
    ...
    file.write(f"Totalul preturilor locuintelor din {city} este: {total_price} lei\n\n")
    ...
    file.write(f"Media rating-urilor locuintelor din {city} este : {avg_rating}\n")
    file.write(f"Cea mai slaba evaluata locuinta din {city} este '{min_rating_hotel[:-1]}' cu nota {min_rating}\n")
    file.write(f"Cea mai bine evaluata locuinta din {city} este '{max_rating_hotel[:-1]}' cu nota {max_rating}\n\n")
    ...
    file.write("EVALUARI:\n")
    file.write(f"In medie, o locuinta din {city} are {avg_evals} evaluari\n")
    file.write(f"Cea mai putin evaluata locuinta din {city}, cu {min_evals} evaluari este '{min_evals_hotel[:-1]}'\n")
    file.write(f"Cea mai evaluata locuinta din {city}, cu {max_evals} evaluari este '{max_evals_hotel[:-1]}'\n\n")
    ...
    df['pret_normalizat'] = 1 - ((preturi - preturi.min()) / (preturi.max() - preturi.min()))
